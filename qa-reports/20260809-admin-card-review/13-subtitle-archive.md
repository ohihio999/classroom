...
- **卡片 ID：** `subtitle-archive`
- **正式站：** https://my-teaching-tools-2b36c.web.app/admin/
- **本機來源：** `C:\Users\admin\Desktop\classroom\tools\admin\index.html:660`
- **動作：** `modal` → `openSubtitleArchive`
- **問題等級：** 低
- **分類：** 內容

## 1. 結論

「字幕歸檔」範圍不明，沒有強調是課程字幕與人工校對版替換自動字幕。

## 2. 實際檢查證據

Modal 函式與 Arctime 校對字幕替換流程說明存在。
...
|---|---|---|
| 標題 | 字幕歸檔 | 課程字幕歸檔 |
| 說明 | Arctime 校對版字幕換掉自動產的 srt | 用 Arctime 校對版 SRT 取代自動字幕並保留整理結構 |
| 連結／入口 | `openSubtitleArchive` | `openSubtitleArchive` |
...
可在 Modal 加「只預覽、不覆蓋」第一步與原 SRT 備份證據。
