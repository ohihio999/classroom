# 16｜圖片合併 PDF（本機）｜卡片改善報告

- **卡片 ID：** `image-to-pdf`
- **正式站：** https://my-teaching-tools-2b36c.web.app/admin/
- **本機來源：** `C:\Users\admin\Desktop\classroom\tools\admin\index.html:668`
- **動作：** `modal` → `openImgPdf`
- **問題等級：** 中
- **分類：** UX／功能邊界
...
原名稱沒有說是整個資料夾合併；「重新編號歸檔」具破壞性，卡片應更清楚提示本機服務與行為。

## 2. 實際檢查證據

8767 服務實跑 HTTP 200；選檔、開始轉換、輪詢進度函式存在。因會建立 PDF 並搬／重新編號圖片，本輪沒有對使用者檔案做實際轉換。
...
|---|---|---|
| 標題 | 圖檔轉 PDF | 圖片合併 PDF（本機） |
| 說明 | 整夾圖片合成 PDF＋重新編號歸檔；服務沒開就按 🚀 | 整夾圖片合成 PDF 並依序編號歸檔；需啟動 8767 |
| 連結／入口 | `openImgPdf` | `openImgPdf` |
...
正式執行前應加入不可逆操作摘要與原圖備份狀態，並以測試資料夾做端到端驗收。
